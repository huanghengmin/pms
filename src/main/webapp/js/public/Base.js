...
var ctop = window.top;
function postRequest(url, params, classname, method, showwait, success, error, complete, $form) {
    if (showwait) {
        popWaitProcess("请稍侯!", "正在处理数据");
    }
    var token = "";
    if (url.toLowerCase().indexOf("/pms/") >= 0) {
        token = getRequestToken(url, classname, method);
    }
    $.ajax({
        url: url + classname + "!" + method + ".action?ran=" + Math.random() + "&reqtk=" + token,
        type: "POST",
        dataType: "text",
        data: params,
...
/*
 *printPreview 针对打印预览处理
 */
function openWin(url, width, height, div, printPreview,callback,callbackdata) {
    if (div == undefined || div == "") {
        div = "pwin";
    }
    if (printPreview == undefined || printPreview == "") {
        printPreview = 1;
    }
    ...
    if (div=="paywin"&&callback != undefined && typeof (callback) == "function")
    {
        //支付页面 回调处理
        $(win).find("a").click(function () {
...
//是否启用自定义账单 2015-12-10
function IsDefineBill() {
    var result = false;
    var userAgent = window.navigator.userAgent.toLowerCase();
    if (userAgent == "jchotelclient") {//判断是不是客户端
        $.ajax({
            url: "/services/basicservice.aspx?classname=DefineBillUsl&method=GetEnabledDefineBill",
            type: "post",
            async: false,
            datatype: "json",
